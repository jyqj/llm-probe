# Claude Code — claude-sonnet-4-6 基线
...
| 结构完整性 | 25/25 | 35/35 |
| 签名校验 | 18.33333333333333/20 | 11/12 |
| 行为验证 | 17.5/20 | 7/8 |
| 多模态能力 | 10/10 | 2/2 |

## 逐 Probe 结果

### precheck — 基线流式探针 [PARTIAL 11/17] (2119ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `headers` | FAIL | 缺少 13/13 | missing 13/13 ratelimit headers |
| `request_id` | PASS | UUID 格式 | X-Request-Id UUID format OK: 9b9ebae4-3499-4f19-9... |
...
| `signature_length` | PASS | 727 bytes | signature 727 bytes OK |
...
| `tag_replay` | PASS | tag found in response | tag found in response: 348b4d3aaaac52d9 |
| `cache_fake` | PASS | create=6753 read=18693 | cache values non-zero: create=6753 read=18693 |
...
### mini_probe — 极简非流式探针 [PASS] (2004ms)
...
### identity_probe — 身份识别探针 [PASS] (19031ms)
...
### self_intro — 结构化自述探针 [PARTIAL 5/6] (7328ms)
...
### logic — 逻辑推理探针 [PASS] (30988ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `logic_answer` | PASS | found keyword: 热 | contains heat/warm method |

### hidden_prompt — 隐藏 Prompt 检测 [PARTIAL 0/1] (3624ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `hidden_prompt` | FAIL | input_tokens=23 | input_tokens=23 (expected ≤20 for bare 'hi'), likely hidden system prompt injected |

### image_ocr — 图片 OCR 探针 [PASS] (4276ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `image_ocr` | PASS | 包含 VQKMPJBH | image OCR correct: VQKMPJBH |

### pdf_extract — PDF 提取探针 [PASS] (2787ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `pdf_extract` | PASS | 包含 5QAWU2B5 | PDF text correct: 5QAWU2B5 |

### magic_refusal — 拒答字符串探针 [PASS] (3082ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `magic_refusal` | PASS | end_turn | stop_reason=end_turn (部分渠道/tier 不触发 refusal) |

### effort_thinking — Effort 级别思考探针 [PASS] (38478ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `effort_high_thinking` | PASS | 2 content blocks | effort 请求被接受，响应含有效 content (含 thinking 块) |
| `effort_high_signature` | PASS | 1256 bytes | signature 有效 (1256 bytes) |
...
### signature_reject — 空签名拒绝探针 [PARTIAL 0/1] (2432ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `signature_empty_rejected` | FAIL | 200 | 状态码 200, 期望 400 |

### bash_tool — Bash 工具探针 [PASS] (2682ms)
...
### minimal_tokens — 最小 Token 计费探针 [PARTIAL 1/2] (2253ms)
