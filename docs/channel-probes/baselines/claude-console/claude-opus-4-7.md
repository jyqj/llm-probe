# Claude Console — claude-opus-4-7 基线
...
### precheck — 基线流式探针 [PASS] (2499ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `headers` | PASS | 13/13 齐全 | all Anthropic ratelimit headers present |
| `request_id` | PASS | req_01 前缀 | Request-Id format OK: req_011Cb2rEasGbebdq... |
...
| `server_timing` | PASS | X-Envoy=1656 | X-Envoy-Upstream-Service-Time present: 1656 |
| `cf_ray_format` | PASS | 9fbb97c6ace05397-LAX | Cf-Ray format OK: 9fbb97c6ace05397-LAX |
...
| `tag_replay` | PASS | tag found in response | tag found in response: 88a81e7508bcfbd0 |
| `cache_fake` | PASS | create=34289 read=0 | cache values non-zero: create=34289 read=0 |
| `message_start_usage` | PASS | input_tokens=528 | message_start usage OK: input_tokens=528 |
| `sse_ping_position` | PASS | ping after content_block_start | ping after content_block_start OK |
...
### mini_probe — 极简非流式探针 [PASS] (1748ms)
...
| `small_ephemeral_zero` | PASS | 5m=8700 1h=0 | ephemeral_5m=8700 ephemeral_1h=0 (cache_control used) |
| `small_cache_zero` | PASS | create=8700 read=0 | cache create=8700 read=0 (cache_control used) |
| `token_budget` | PASS | input_tokens=12 | input_tokens=12 within budget (≤80) |

### identity_probe — 身份识别探针 [PASS] (9743ms)
...
### self_intro — 结构化自述探针 [PARTIAL 5/6] (13390ms)
...
| `structured_name_correct` | PASS | 包含 "Henry Larsson" | name correct: Henry Larsson |
| `structured_stop_reason` | PASS | end_turn | stop_reason=end_turn OK |

### tool_use — 工具调用探针 [PASS] (20355ms)
...
### logic — 逻辑推理探针 [PASS] (20306ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `logic_answer` | PASS | found keyword: 热 | contains heat/warm method |

### hidden_prompt — 隐藏 Prompt 检测 [PASS] (1687ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `hidden_prompt` | PASS | input_tokens=13 | input_tokens=13 (clean, no hidden prompt) |

### image_ocr — 图片 OCR 探针 [PASS] (3943ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `image_ocr` | PASS | 包含 DVJHXJMD | image OCR correct: DVJHXJMD |

### pdf_extract — PDF 提取探针 [PASS] (2148ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `pdf_extract` | PASS | 包含 AAZUHJW6 | PDF text correct: AAZUHJW6 |

### magic_refusal — 拒答字符串探针 [PASS] (2954ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `magic_refusal` | PASS | end_turn | stop_reason=end_turn (部分渠道/tier 不触发 refusal) |

### effort_thinking — Effort 级别思考探针 [PASS] (44838ms)
...
### signature_reject — 空签名拒绝探针 [PASS] (335ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `signature_empty_rejected` | PASS | 400 | 状态码 400, 空 signature 被正确拒绝 |

### bash_tool — Bash 工具探针 [PASS] (2279ms)
...
### minimal_tokens — 最小 Token 计费探针 [PASS] (1807ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `minimal_input_tokens` | PASS | 13 | input_tokens=13 (范围 8-16 OK) |
