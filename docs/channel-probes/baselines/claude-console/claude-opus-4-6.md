# Claude Console — claude-opus-4-6 基线
...
### precheck — 基线流式探针 [PASS] (2293ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `headers` | PASS | 13/13 齐全 | all Anthropic ratelimit headers present |
| `request_id` | PASS | req_01 前缀 | Request-Id format OK: req_011Cb2rETU2gJLuc... |
...
| `server_timing` | PASS | X-Envoy=1438 | X-Envoy-Upstream-Service-Time present: 1438 |
| `cf_ray_format` | PASS | 9fbb97bc1fe54383-LAX | Cf-Ray format OK: 9fbb97bc1fe54383-LAX |
...
| `signature_length` | PASS | 421 bytes | signature 421 bytes OK |
...
| `tag_replay` | PASS | tag found in response | tag found in response: 668e2f9e79228984 |
| `cache_fake` | PASS | create=25086 read=0 | cache values non-zero: create=25086 read=0 |
| `message_start_usage` | PASS | input_tokens=400 | message_start usage OK: input_tokens=400 |
| `sse_ping_position` | PASS | ping after content_block_start | ping after content_block_start OK |
...
### mini_probe — 极简非流式探针 [PASS] (1366ms)
...
| `small_ephemeral_zero` | PASS | 5m=6296 1h=0 | ephemeral_5m=6296 ephemeral_1h=0 (cache_control used) |
| `small_cache_zero` | PASS | create=6296 read=0 | cache create=6296 read=0 (cache_control used) |
| `token_budget` | PASS | input_tokens=7 | input_tokens=7 within budget (≤80) |

### identity_probe — 身份识别探针 [PASS] (19920ms)
...
### self_intro — 结构化自述探针 [PARTIAL 5/6] (5038ms)
...
| `structured_name_correct` | PASS | 包含 "Bob Fischer" | name correct: Bob Fischer |
| `structured_stop_reason` | PASS | end_turn | stop_reason=end_turn OK |

### tool_use — 工具调用探针 [PASS] (19771ms)
...
### logic — 逻辑推理探针 [PASS] (31085ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `logic_answer` | PASS | found keyword: 热 | contains heat/warm method |

### hidden_prompt — 隐藏 Prompt 检测 [PASS] (1649ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `hidden_prompt` | PASS | input_tokens=8 | input_tokens=8 (clean, no hidden prompt) |

### image_ocr — 图片 OCR 探针 [PASS] (2723ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `image_ocr` | PASS | 包含 JVYPXL88 | image OCR correct: JVYPXL88 |

### pdf_extract — PDF 提取探针 [PASS] (2600ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `pdf_extract` | PASS | 包含 JLLK49MY | PDF text correct: JLLK49MY |

### magic_refusal — 拒答字符串探针 [PASS] (2229ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `magic_refusal` | PASS | end_turn | stop_reason=end_turn (部分渠道/tier 不触发 refusal) |

### effort_thinking — Effort 级别思考探针 [PASS] (37487ms)
...
### signature_reject — 空签名拒绝探针 [PASS] (269ms)

| Check | 状态 | 实际值 | 详情 |
|---|---|---|---|
| `signature_empty_rejected` | PASS | 400 | 状态码 400, 空 signature 被正确拒绝 |

### bash_tool — Bash 工具探针 [PASS] (2242ms)
...
### minimal_tokens — 最小 Token 计费探针 [PASS] (2160ms)
